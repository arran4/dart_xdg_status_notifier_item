...
- [x] Support markup AND escaping markup when it's not desired (based on the `StatusNotifierItem` Markup spec).
- [x] Implement data sanitizers for icons and other inputs (based on the `StatusNotifierItem` Icons spec and real-world implementations).
## Desktop Environments (KDE, GTK, Ayatana, etc.)
- [x] KDE (`org.kde.StatusNotifierWatcher`): Implement KDE baring in mind that we are increasing the bar as to the feature set we want to support.
...
- [x] Ubuntu (`com.canonical.dbusmenu`): Fully support canonical dbusmenu extensions.
